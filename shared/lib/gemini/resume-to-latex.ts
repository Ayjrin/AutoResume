import { getGeminiClient, safetySettings, generationConfig, base64ToGenerativePart } from './client';
import fs from 'fs';
...
const getResumeTemplate = (): string => {
  try {
    const templatePath = path.join(process.cwd(), 'shared', 'templates', 'resume-template.txt');
 ...
 */
export async function convertResumeToLatex(options: ConvertResumeToLatexOptions): Promise<string> {
  try {
    ...
    if (options.multipleFiles && options.base64FilesData && options.mimeTypes) {
      ...
      if (options.fileNames && options.fileNames.length > 0) {
        const fileList = options.fileNames.map(name => `- ${name}`).join('\n');
        contentParts.push(`Note: The following documents have been provided:\n${fileList}\n\nPlease extract information from all documents and create a comprehensive resume.`);
      }
    } else if (options.base64FileData && options.mimeType) {
      // Handle single file for backward compatibility
      const part = await base64ToGenerativePart(options.base64FileData, options.mimeType);
      contentParts.push(part);
    } else {
      throw new Error('Invalid options: Either base64FileData or base64FilesData must be provided');
    }
    ...
    throw new Error(`Failed to convert resume to LaTeX: ${error instanceof Error ? error.message : String(error)}`);
  }
}
